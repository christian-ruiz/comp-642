...
def rmsprop(theta, dtheta, config=None):
  ...
  #############################################################################
  config['cache'] = config['decay_rate']*config['cache'] + (1-config['decay_rate'])*dtheta*dtheta
  delta_theta = - (config['learning_rate'])*dtheta/(config['epsilon']+np.sqrt(config['cache']))
...
def adam(theta, dtheta, config=None):
  ...
  dtheta = -(config['learning_rate']*corrected_m)/(config['epsilon']+np.sqrt(corrected_v))
