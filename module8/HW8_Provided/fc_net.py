...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dout1, grads['theta2'], grads['theta2_0'] = affine_backward(dout2,cache2)
...
#         theta_id = str(i+1)
#         self.params['theta'+theta_id] = np.random.normal(0, weight_scale,(hidden_dims[i-1],hidden_dims[i]))
#         self.params['theta'+theta_id+'_0'] = np.zeros(hidden_dims[i])
#     self.params['theta'+str(len(hidden_dims)+1)] = np.random.normal(0, weight_scale, (hidden_dims[-1],num_classes))
...
#       theta_id = str(i + 1)
#       out, cache = affine_relu_forward(out_list[-1], self.params['theta' + theta_id],
#                                        self.params['theta' + theta_id + '_0'])
#       out_list.append(out)
#       cache_list.append(cache)
#       if self.use_dropout:
#         out_dropout, cache_dropout = dropout_forward(out, dropout_param=self.dropout_param)
#         cache_list.append(cache_dropout)
#         out_list.append(out_dropout)

#     theta_id = str(self.num_layers - 1)
#     out_last, cache_last = affine_forward(out_list[-1], self.params['theta' + theta_id],
#                                           self.params['theta' + theta_id + '_0'])
#     cache_list.append(cache_last)
...
#     # of 0.5 to simplify the expression for the gradient.                      #
#     ############################################################################
#     loss, dout_softmax = softmax_loss(out_list[-1], y)

#     # add the regularization to loss
#     # print('before regularization loss: {} '.format(loss))
#     l2 = 0
#     for i in range(self.num_layers-1):
#       theta_id = str(i+1)
#       l2 += np.sum(self.params['theta'+theta_id] * self.params['theta'+theta_id])
#     loss += ((1*self.reg)/2)*l2
#     # print('after regularization reg = {} loss = {} '.format(self.reg, loss))

#     # print('shape dout_softmax {} '.format(dout_softmax.shape))

#     dout_last, grads['theta'+str(self.num_layers-1)], \
#     grads['theta'+str(self.num_layers-1)+"_0"] = affine_backward(dout_softmax, cache_list[-1])

#     len('length of cache_list = {}'.format(cache_list))
#     cache_list.pop(-1)

#     for i in reversed(range(0,self.num_layers-2)):

#       if self.use_dropout:
#         dout_dropout = dropout_backward(dout_last,cache_list[-1])
#         cache_list.pop(-1)
#         dout_last = dout_dropout

#       theta_id = str(i+1)
#       current_dout , grads['theta'+theta_id], grads['theta'+theta_id+"_0"] \
#         = affine_relu_backward(dout_last,cache_list[-1])
#       dout_last = current_dout
#       cache_list.pop(-1)

#     for i in range(self.num_layers-1):
#       theta_id = str(i+1)
#       grads['theta'+theta_id] += self.reg * self.params['theta'+theta_id]
